...
| `my_pet` | `ui/pet/room/PetRoomScreen` | My Pet Room: scene phòng full-screen, top bar biển gỗ + Settings, Music và shortcut Pet Store xếp bên phải, sheet ba tab My Pet/Food/Room. Tab My Pet liệt kê pet đã sở hữu (pack đã cài) kèm ô `+` mở Pet Store; mỗi card luôn có badge `Active` hoặc `Inactive`. Chạm một pet mở panel chi tiết (tên, breed, ngày nhận nuôi, trạng thái Active/Inactive + switch, thanh Energy); pet cuối cùng cũng có thể chuyển Inactive. Chỉ pet Active xuất hiện trong scene/overlay, còn pet Inactive vẫn ở roster để bật lại. Energy dùng mốc nhận nuôi hoặc lần ăn gần nhất, tụt 1%/phút và cập nhật trực tiếp khi panel mở. Pet mới từ Store mặc định ON ở slot Mixed trống đầu tiên. Tab Food tiêu một phần ăn để hồi Energy và nút `+` quay về Pet Store; tab Room chọn background từ catalog và persist, card vừa tap hiển thị scrim/progress trong lúc tải và trở lại trạng thái download nếu lỗi. Xoá pet bằng nút X trên card kèm dialog xác nhận; pet bị xoá mất luôn slot overlay. Pet nổi tạm tắt khi màn này mở |
| `grant_permissions` | `ui/settings/permissions/GrantPermissionsScreen` | Route/screen được giữ nhưng option vào từ Mine đang tạm ẩn. App bar collapse theo list; native ad ghim đáy. Route mặc định ưu tiên Accessibility, còn `requiredTarget=overlay` ưu tiên Draw over apps sau shared disclosure. CTA bắt buộc mới mở system settings; các row còn lại giữ state thật và description theo tác dụng trong app. Accessibility chưa cấp đi qua shared consent disclosure rồi màn How to use. Các mục đọc lại trạng thái ở `ON_RESUME`; battery optimization dùng `ACTION_IGNORE_BATTERY_OPTIMIZATIONS_SETTINGS` thay vì quyền `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` bị Play hạn chế, và row đó chỉ hiện khi exemption thật sự có tác dụng (xem `PET_OVERLAY.md` cho đủ sáu tín hiệu); ROM có allowlist riêng của hãng thì có thêm row Allow auto-start dẫn tới đúng màn đó. Mọi intent đều mở qua launcher có fallback, vì màn hệ thống có thể không tồn tại trên ROM rút gọn và activity vendor resolve được vẫn có thể không `exported` |
