...
| `grant_permissions` | `ui/settings/permissions/GrantPermissionsScreen` | Route/screen được giữ nhưng option vào từ Mine đang tạm ẩn. Ba nhóm quyền theo Figma: accessibility (badge Required/Allowed + Go to Settings), overlay và ignore battery optimization, notification. Accessibility chưa cấp đi qua shared consent disclosure rồi màn How to use; overlay chưa cấp đi qua shared overlay disclosure. Các mục đọc lại trạng thái ở `ON_RESUME`; battery optimization dùng `ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS` thay vì quyền `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` bị Play hạn chế, và row đó chỉ hiện khi exemption thật sự có tác dụng (xem `PET_OVERLAY.md` cho đủ sáu tín hiệu); ROM có allowlist riêng của hãng thì có thêm row Allow auto-start dẫn tới đúng màn đó. Mọi intent đều mở qua launcher có fallback, vì màn hệ thống có thể không tồn tại trên ROM rút gọn và activity vendor resolve được vẫn có thể không `exported` |
