...
| `my_pet` | `ui/pet/room/PetRoomScreen` | My Pet Room: scene phòng full-screen, top bar biển gỗ + Settings, Music và shortcut Pet Store xếp bên phải, sheet ba tab My Pet/Food/Room. Tab My Pet liệt kê pet đã sở hữu (pack đã cài) kèm ô `+` mở Pet Store; mỗi card luôn có badge `Active` hoặc `Inactive`. Chạm một pet mở panel chi tiết (tên, breed, ngày nhận nuôi, trạng thái Active/Inactive + switch, thanh Energy); pet cuối cùng cũng có thể chuyển Inactive và khi không có pet active overlay không hiển thị gì. Pet mới từ Store mặc định ON ở slot Mixed trống đầu tiên. Tab Food tiêu một phần ăn để hồi Energy và nút `+` quay về Pet Store; tab Room chọn background từ catalog và persist, card vừa tap hiển thị scrim/progress trong lúc tải và trở lại trạng thái download nếu lỗi. Xoá pet bằng nút X trên card kèm dialog xác nhận; pet bị xoá mất luôn slot overlay. Pet nổi tạm tắt khi màn này mở |
