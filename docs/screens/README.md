...
| `pet_store` | `ui/pet/store/PetStoreScreen` | Tab Pet Store: shared Home header/switch, My Pet hero mở My Pet Room, pet/food tabs; pet tab có category ngang lấy động từ catalog và lọc grid. Reward sheet hiển thị scrim/progress trên preview trong lúc verified download, sau đó Lottie unlock reveal chạy special movement clip của pack và tiếp tục name/toast; food persistence TODO sau My Pet |
