...
/**
 * Cache-first reader for `json/battery-troll.json` and its `troll/` assets: the same private
 * repository contract as the pet, Battery and room catalogs (bearer token from Remote Config,
 * ETag revalidation with a 24h TTL, rate-limit backoff, size + SHA-256 verified downloads).
 */
@Singleton
class GithubBatteryTrollCatalogClient @Inject constructor(
    @param:ApplicationContext private val context: Context
) {
    private val catalogDirectory = File(context.filesDir, "battery_troll_catalog_remote").also {
        it.mkdirs()
    }
    private val catalogFile = File(catalogDirectory, "battery-troll.json")
    private val metadataFile = File(catalogDirectory, "metadata.json")
    private val assetDirectory = File(context.filesDir, "battery_troll_catalog_assets").also {
        it.mkdirs()
    }

    fun readCachedCatalog(): CachedBatteryTrollCatalog? = runCatching {
        val json = catalogFile.takeIf(File::isFile)?.readText(Charsets.UTF_8)
            ?: return@runCatching null
        CachedBatteryTrollCatalog(json, readMetadata())
    }.getOrNull()

    fun readCatalogCacheMetadata(): BatteryTrollCatalogCacheMetadata = readMetadata()

    fun shouldRefreshCatalog(metadata: BatteryTrollCatalogCacheMetadata): Boolean =
    ...
    fun fetchCatalog(etag: String?): BatteryTrollCatalogFetchResult = runCatching {
        val connection = openConnection(BatteryTrollServerConfig.CATALOG_URL)
        try {
            ...
            when (connection.responseCode) {
                HttpURLConnection.HTTP_OK -> BatteryTrollCatalogFetchResult.Updated(
                ...
                HttpURLConnection.HTTP_NOT_MODIFIED -> BatteryTrollCatalogFetchResult.NotModified(
                    connection.getHeaderField(ETAG_HEADER) ?: etag
                )

                HTTP_TOO_MANY_REQUESTS, HttpURLConnection.HTTP_FORBIDDEN ->
                    BatteryTrollCatalogFetchResult.RateLimited(
                ...
                else -> BatteryTrollCatalogFetchResult.Failed
            }
        } finally {
            connection.disconnect()
        }
    }.getOrDefault(BatteryTrollCatalogFetchResult.Failed)

    fun cacheCatalog(json: String, etag: String?): Boolean = runCatching {
        val temporary = File(catalogDirectory, "battery-troll.json.tmp")
        temporary.writeText(json, Charsets.UTF_8)
        replace(temporary, catalogFile)
        check(
            writeMetadata(
                BatteryTrollCatalogCacheMetadata(
                    etag = etag,
    ...
    /** Downloads the asset when needed and returns it only once size and hash both match. */
    @Synchronized
    fun materializeAsset(
        ...
        temporary.delete()
        val downloaded = request(BatteryTrollServerConfig.resolve(relativePath)) { connection ->
                ...
                FileOutputStream(temporary).use { output ->
                    ...
                    while (true) {
                        ...
                        if (totalBytes > expectedSizeBytes || totalBytes > MAX_ASSET_BYTES) {
                            error("Battery Troll asset exceeds its declared size")
                        }
    ...
    private fun readMetadata(): BatteryTrollCatalogCacheMetadata = runCatching {
        val root = metadataFile.takeIf(File::isFile)
            ?.readText(Charsets.UTF_8)
            ?.let(::JSONObject)
            ?: return@runCatching BatteryTrollCatalogCacheMetadata()
        BatteryTrollCatalogCacheMetadata(
    ...
    }.getOrDefault(BatteryTrollCatalogCacheMetadata())

    private fun writeMetadata(metadata: BatteryTrollCatalogCacheMetadata): Boolean = runCatching {
        val root = JSONObject()
    ...
    private fun <T> request(url: String, block: (HttpURLConnection) -> T): T? = runCatching {
        ...
        try {
            ...
            if (connection.responseCode !in 200..299) {
                error("Battery Troll server returned HTTP ${connection.responseCode}")
            }
            block(connection)
        } finally {
            connection.disconnect()
...
data class CachedBatteryTrollCatalog(
    val json: String,
    val metadata: BatteryTrollCatalogCacheMetadata
)

data class BatteryTrollCatalogCacheMetadata(
    val etag: String? = null,
    val lastValidatedAtEpochMillis: Long = 0L,
    val retryAfterEpochMillis: Long = 0L
)

sealed interface BatteryTrollCatalogFetchResult {
    data class Updated(val json: String, val etag: String?) : BatteryTrollCatalogFetchResult
    data class NotModified(val etag: String?) : BatteryTrollCatalogFetchResult
    data class RateLimited(val retryAtEpochMillis: Long) : BatteryTrollCatalogFetchResult
    data object Failed : BatteryTrollCatalogFetchResult
}
